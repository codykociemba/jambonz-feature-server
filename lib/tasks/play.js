        ...
        ep.once('playback-start', (evt) => {
          ...
          this.cs.stickyEventEmitter?.once('uuid_break', (t) => {
            if (t?.taskId === this.taskId) {
              ...
              this.ep.api('uuid_break', `${this.ep.uuid} all`).catch((err) => this.logger.info(err, 'Error killing audio'));
        ...
        if (cs.isInConference) {
          ...
          for (const playUrl of urls) {
            ...
            if (this.seekOffset >= 0) {
              file = {file: playUrl, seekOffset: this.seekOffset};
              this.seekOffset = -1;
            }
            result = await ep.play(file);
            playbackSeconds += parseInt(result.playbackSeconds);
            playbackMilliseconds += parseInt(result.playbackMilliseconds);
          }
          if (result && (this.killed || !this.loop || completed)) {
